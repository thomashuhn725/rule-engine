...
## Laradock Container Artisan Commands
- All `php artisan` commands should be executed via the "laradock-workspace-1" Docker container
- By default, the working directory within the container is going to be `/var/www`, so you will need to `cd` into the project root before executing the Artisan command
- For example: `docker exec laradock-workspace-1 sh -c 'cd {project-root} && php artisan {name}'`
